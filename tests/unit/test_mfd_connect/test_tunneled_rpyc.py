...
class TestTunneledRPyCConnectionIPv6:
    ...
    def test_tunneled_rpyc_passes_ipv6_true(self, mock_super_init, _mock_set_proc, _mock_log, _mock_bg):
        ...
        mock_remote_rpyc_connect.assert_called_once_with(
            ...
            ipv6=True,
            service=mock_tunnel_conn.modules.rpyc.ClassicService,
            keepalive=True,
    ...
    @patch.object(TunneledRPyCConnection, "_set_process_class")
    @patch.object(RPyCConnection, "__init__", autospec=True, return_value=None)
    def test_tunneled_rpyc_starts_background_serving_thread(self, mock_super_init, _mock_set_proc, _mock_log, mock_bg):
        """Test that the background serving thread is started when enabled."""
        mock_tunnel_conn = MagicMock()
        mock_tunnel_conn.closed = False
        mock_remote_rpyc = mock_tunnel_conn.modules.rpyc
        mock_remote_rpyc.connect.return_value = MagicMock(closed=False)

        def fake_super_init(self_inner, *args, **kwargs):
            self_inner._connection = mock_tunnel_conn
            self_inner._enable_bg_serving_thread = True
            self_inner.path_extension = None
        ...
        TunneledRPyCConnection(
            ip="10.10.10.10",
            jump_host_ip="10.10.10.1",
            enable_bg_serving_thread=True,
        )

        mock_bg.assert_called_once_with(mock_remote_rpyc.connect.return_value)
