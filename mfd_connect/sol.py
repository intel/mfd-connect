...
class SolConnection(Connection):
        ...
        for attempt in range(1, retry_count + 1):
            ...
            try:
                ...
                if process is not None:
                    try:
                        # PopenSpawn has no close() - kill() sends a signal via the underlying subprocess.
                        process.kill(getattr(signal, "SIGKILL", signal.SIGTERM))
                    except (ProcessLookupError, OSError):
                        # Process already gone, or killing it failed for another OS-level reason (e.g.
                        # permissions) - not fatal here, the fallback cleanup below will handle it.
                        pass
                continue
    ...
    def _kill_defunct_ipmiutil_processes(self) -> None:
        ...
        Process listing uses 'ps -o pid,stat --no-headers -C <tool_name>': '-C <name>'
        selects processes by command name and, on its own, already overrides the default
        "only same-terminal processes" restriction of 'ps', matching system-wide.

        Only processes in STAT='T' (stopped by job control signal) are considered defunct
        and killed, healthy ipmiutil is usually 'S' or 'S+' (interruptible sleep).

        Finding no defunct process is not treated as fatal, it simply means there's
        nothing to clean up.

        :raises SolException: if process listing fails or kill operation fails
        """
        try:
            ps_process = pexpect.popen_spawn.PopenSpawn(
                f"ps -o pid,stat --no-headers -C {self._ipmi_tool_name}", timeout=30
            )
            # Accept TIMEOUT as well as EOF: if 'ps' hasn't finished within 30s, use whatever
            # partial output was captured in ps_process.before instead of raising an exception.
            ps_process.expect([pexpect.EOF, pexpect.TIMEOUT])
        ...
        if not defunct_pids:
            logger.log(
                level=log_levels.MODULE_DEBUG,
                msg=f"No defunct (stopped, STAT=T) {self._ipmi_tool_name} process(es) found - nothing to clean up.",
            )
            return
